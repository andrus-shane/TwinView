...
- Focus a unit without clicking through the lab: `window.__viewer.onSelectUnit('u06')` (ids `u01`..`u12`, same path a 3D click takes, so the app wires the console canvas). Park the camera on the console with `__viewer.flyTo(pos, target, false)`; the CAD screen overlay mesh is named `__console_screen__` (poll for it under the focused bay, and for `camAnim === null`).
- The Browser pane's `zoom` region crop isn't supported; it returns the full screenshot.
